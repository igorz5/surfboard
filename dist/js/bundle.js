...
var boardsList = $(".boards__list");
boardsList.slick({
  prevArrow: ".boards__prev",
  nextArrow: ".boards__next" // variableWidth: true,

});
"use strict";

var fixedMenuItems = $(".fixed-menu__item");
var fixedMenuLinks = $(".fixed-menu__link");
...
function updateFixedMenu() {
  fixedMenuItems.each(function () {
    ...
    var link = $(this).find(".fixed-menu__link");
    var idx = getSectionIndexByName(link.attr("data-section"));
...
var fullscreenNavbar = $(".fullscreen-navbar");
var navbarClose = $(".fullscreen-navbar__close");
var navbarItems = $(".fullscreen-navbar .navbar__item");
...
ymaps.ready(function () {
  ...
  var coords = [[55.76, 37.64], [55.77, 37.66], [55.78, 37.62], [55.79, 37.67]];
...
var menuBtns = $(".menu__btn");
menuBtns.on("click", function (e) {
  var elem = $(this).closest(".menu__item");
...
function scrollToPage(idx) {
  if (idx >= 0 && idx <= sections.length - 1) {
    var offset = idx * 100;
    content.css("transform", "translateY(".concat(-offset, "%)"));
    currentPage = idx;
    updateFixedMenu();
    lockScroll();
...
function getSectionIndexByName(name) {
  var arr = sections.toArray();
  return arr.findIndex(function (elem) {
    return $(elem).attr("data-section") === name;
  });
}
...
var orderSubmit = $(".order__btn-submit");
var orderFieldNames = ["name", "phone", "house", "flat", "comment"];
var orderFields = {};
orderFieldNames.forEach(function (name) {
  var field = orderForm.find(":input[name=".concat(name, "]"), "textarea[name=".concat(name, "]"));
...
orderForm.on("submit", function (e) {
  ...
  if (checkFormValidity()) {
    ...
    xhr.addEventListener("load", function () {
      ...
      var isError = data.status === 0;
      openModal(data.message, isError, function () {
        orderForm.trigger("reset");
      });
    });
    xhr.addEventListener("error", function () {
      var msg = "Упс... Произошла ошибка";
      openModal(msg, true, function () {
        orderForm.trigger("reset");
      });
    });
...
function checkFieldValidity(field) {
  var elem = field[0];
  var isValid = elem && field[0].checkValidity();
...
function checkFormValidity() {
  var isValid = true;
  Object.values(orderFields).forEach(function (field) {
...
var playBtn = $(".player__play-btn");
var playerSplash = $(".player__splash");
var playerControls = $(".player__controls");
var playerProgress = $(".player__progress");
var playerVolumeLine = $(".player__volume-line");
var playerProgressCurrent = $(".player__timeline .player__current");
var playerVolumeCurrent = $(".player__volume .player__current");
var playerProgressHandler = $(".player__progress-handler");
var playerVolumeHandler = $(".player__volume-handler");
var playerVolumeBtn = $(".player__volume-btn");
var player = null;
var targetPressed = null;
var lastVolume = 0;
var preventProgressUpdate = false;
...
function onPlayerReady() {
  ...
  var getOnMouseDown = function getOnMouseDown(elem) {
    return function () {
      targetPressed = elem;
    };
  };
  ...
  var onMouseMove = function onMouseMove(e) {
    if (targetPressed != null) {
      ...
      if (targetPressed.is(playerProgressHandler)) {
        var x = mx - playerProgress.offset().left;
        var width = playerProgress.width();
        var f = x / width;
        var dur = player.getDuration();
        setPlayerElapsedTime(f * dur);
      } else if (targetPressed.is(playerVolumeHandler)) {
        var _x = mx - playerVolumeLine.offset().left;

        var _width = playerVolumeLine.width();

        var _f = _x / _width;

        player.setVolume(_f * 100);
      }
    }
  };
  ...
  playerProgress.on("click", function (e) {
    var x = e.pageX - playerProgress.offset().left;
    var width = playerProgress.width();
    var f = x / width;
    var dur = player.getDuration();
    setPlayerElapsedTime(f * dur);
  ...
  setInterval(function () {
    if (player.getPlayerState() === YT.PlayerState.PLAYING && !preventProgressUpdate) {
...
function updatePlayerProgress(time) {
  ...
  var f = time / dur;
  playerProgressCurrent.css("width", "".concat(f * 100, "%"));
  preventProgressUpdate = true;
}

function updateVolume() {
  var v = player.getVolume();
  playerVolumeCurrent.css("width", "".concat(v, "%"));
}
...
var reviews = $(".reviews__item");
var paginationItems = $(".reviews__pagination-item");
